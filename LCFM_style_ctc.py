...
from networks.recognition import HTRNet
from networks.loss import SupConLoss
...
class ScriptArguments:
    ...
    batch_size: int = 380
    n_epochs: int = 800
    encoder_lr: float = 0.0001  # Lower learning rate for the encoder (1/3 of the UNet rate)
    ...
    cfg_scale_text: float = 2 
    cfg_scale_style: float = 1.5


class WrappedModel_train(ModelWrapper):
                def forward(self, x, t, **extras):
                    ...
                    v_cond = self.model(x_t=x, content=content, style=style, t=t, tag='train')
...
def train_flow_matching_model(rank, world_size, args):
    ...
    try:
        ...
        # === Setup ===
        output_dir = Path(args.output_dir) / "FM_style_cfg"
        ...
        if rank == 0:  # Only create validation loader on main process
            ...
            wid_style_samples = {}
            for batch in tqdm(t_valid_loader, desc="Preloading style samples"):
        ...
        vae = vae.to(device)
        ocr_model = HTRNet(nclasses = 81, vae=True)
        miss, unxep = ocr_model.load_state_dict(torch.load('./pretrained/vae_HTR138.pth', map_location=torch.device('cpu')), strict=False)
        print('load pretrained ocr model from {}'.format('./pretrained/vae_HTR138.pth'))
        ...
        if args.freeze_encoder:
            freeze_content_style_encoder(unet)

        unet_ddp = DDP(unet, device_ids=[rank], output_device=rank, find_unused_parameters=False)
        ...
        if rank == 0 and args.starting_epoch > 0:
            ...
            if args.freeze_encoder and args.starting_epoch >= args.freeze_encoder_epoch:
            ...
            #             param_group['lr'] = args.encoder_lr if not encoder_frozen else 0.0
        ...
        for epoch in range(args.starting_epoch, args.n_epochs):
            ...
            for batch in pbar:
                ...
                target = batch['lbs'].to(device)
                target_lengths = batch['lb_lens'].to(device)
                ...
                with torch.autocast(device_type=device.type, dtype=torch.bfloat16 if device.type == 'cuda' else torch.float32):
                    # First compute MSE loss on velocity prediction
                    unet_ddp.train()  # Ensure in train mode for MSE loss
                ...
                with torch.autocast(device_type=device.type, dtype=torch.bfloat16 if device.type == 'cuda' else torch.float32):
                    # Switch to eval mode for CTC loss to prevent batch norm updates
                    unet_ddp.eval()
                    
                    # Create a clone of x_t
                    x_t_clone = x_t.clone().detach()
                    
                    # Run ODE solver with torch.no_grad() to prevent batch norm updates
                    with torch.no_grad():
                        wrapped_model = WrappedModel_train(unet_ddp)
                        solver = ODESolver(wrapped_model)
                        sample_steps = 51
                        time_steps = torch.linspace(0, 1, sample_steps).to(device)
                        step_size = 0.025
                        
                        # Sample from the ODE solver
                        sol = solver.sample(
                            x_init=x_t_clone,
                            step_size=step_size,
                            method="midpoint",
                            ...
                            content=content_img.detach(),
                            style=style_img.detach()
                        )
                        
                        # Handle the return value correctly
                        if isinstance(sol, list):
                            generated_latent_img = sol[-1]
                        else:
                            generated_latent_img = sol
                    
                    # Detach and create a new tensor for CTC loss
                    generated_latent_img = generated_latent_img.detach().clone().requires_grad_(True)
                    
                    # Compute CTC loss on the detached tensor
                    ocr_model.eval()  # Ensure OCR model is in eval mode
                    rec_out = ocr_model(generated_latent_img)
                    input_lengths = torch.IntTensor(generated_latent_img.shape[0]*[rec_out.shape[0]])
                    ctc_loss = ctc_criterion(F.log_softmax(rec_out, dim=2), target, input_lengths, target_lengths)
                    
                    # Create a proxy loss to connect CTC loss to model parameters
                    # This effectively creates a synthetic gradient
                    weighted_ctc_loss = 0.1 * ctc_loss
                scaler.scale(weighted_ctc_loss).backward()
                    
                    # Combine losses - the MSE loss has full gradient flow,
                    # the CTC loss is using proxy gradients
                loss = loss_mse + weighted_ctc_loss
                ...
                for param_group in optimizer.param_groups:
                    torch.nn.utils.clip_grad_norm_(param_group['params'], max_norm=5)
            ...
            if rank == 0:
                ...
                if epoch == args.starting_epoch or (epoch + 1) % 10 == 0:
                    ...
                    val_mse, val_mse_text = validate(
                        unet_ema.model,
                        path_sampler, t_valid_loader, device, vae, cfg_scale_text=args.cfg_scale_text, cfg_scale_style=args.cfg_scale_style
                    )
                    # avg_valid_loss = val_mse + args.lambda_ctc * val_ctc
                    valid_losses.append(val_mse)
                    valid_losses_text.append(val_mse_text)
                    print(f"Validation Loss: {val_mse:.4f} (MSE: {val_mse:.4f}, MSE_text: {val_mse_text:.4f})")
                    plot_few_shot_style_transfer(args, epoch+1, unet_ema.model, vae, device, 
                                     wid_style_samples, cfg_scale_text=args.cfg_scale_text, cfg_scale_style=args.cfg_scale_style)
                    unet_ddp.train()
                    unet_ema.train(True)
...
def validate(unet, path_sampler, val_loader, device, vae, cfg_scale_text=2, cfg_scale_style=1.5):
    """Validation function focused on generation quality."""
    # unet.eval()
    # encoder.eval()


    total_mse_cond, total_mse_text, total_ctc = 0.0, 0.0, 0.0
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader):
            ...
            for t_val in eval_t_values:
                ...
                v_cond, v_uncond, v_null = unet(x_t, content_img, style_img, t.unsqueeze(1),tag='sample', cfg_scale=cfg_scale_style)
    ...
    return avg_mse_cond, avg_mse_text 

def plot_few_shot_style_transfer(args, epoch, unet, vae, device, wid_style_samples, num_samples=8, k=2, cfg_scale_style=2, cfg_scale_text=1.5):
    os.makedirs("fm_style_cfg", exist_ok=True)
        ...
        for j in range(num_samples):
            ...
            class WrappedModel(ModelWrapper):
                def forward(self, x, t, **extras):
                    ...
                    # Get conditional and unconditional predictions
                    v_cond, v_uncond, v_null = self.model(x_t=x, content=content, style=style, t=t, tag='sample', cfg_scale=cfg_scale_style)
    ...
    plt.tight_layout()
    vis_path = f"fm_style_cfg/epoch_{epoch}_fewshot.png"

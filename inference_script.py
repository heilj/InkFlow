...
class WrappedModelCFG(ModelWrapper):
    ...
    def __init__(self, model, cfg_scale=3):
    ...
    def forward(self, x, t, **extras):
        ...
        s_star_base = optimized_scale(v_cond_flat, v_uncond_flat)
        ...
        if t[0].item() < 1e-5:  # Check if we're at the initial time step
            ...
            # v_guided = v_uncond * s_star_base + self.cfg_scale * (v_cond - v_uncond * s_star_base)
            v_guided = v_cond
        return v_guided
...
def sample(
    ...
    print(f"Generating '{content_text}' with CFG={cfg_scale}")
    sol = solver.sample(
        x_init=x0,
        step_size=step_size,
        method=ode_method,

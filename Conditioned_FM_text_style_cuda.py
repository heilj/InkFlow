...
from networks.encoder import ContentOnlyEncoder
...
wid_style_samples = {}
valid_loader = DataLoader(val_set, batch_size=1, shuffle=True, num_workers=4,
                          collate_fn=Hdf5Dataset.sorted_collect_fn_for_ctc, drop_last=True)
...
content_encoder = ContentOnlyEncoder(output_dim=256)
...
def train_flow_matching_model(args: ScriptArguments):
    ...
    # === Setup ===
    output_dir = Path(args.output_dir) / "FM_text"
    ...
    for epoch in range(args.starting_epoch, args.n_epochs):
        ...
        for batch in pbar:
            ...
            # style_img1 = style_ref[:,0:1,:,:]
            # style_img2 = style_ref[:,1:2,:,:]
            # style1_embed = style_encoder(style_img1)  # [B, D]
            # style2_embed = style_encoder(style_img2)  # [B, D]
            # style_emb = (style1_embed + style2_embed) / 2
            
            # === Process Content ===
            content_feat = content_encoder(content_img)                  # B x C x h x w

            # === Fuse ===
            cond_feat = content_feat                 # B x T x D                  # B x F x h x w
...
def plot_few_shot_style_transfer(args, epoch, num_samples=8, k=1):
    os.makedirs("fm_vis_text", exist_ok=True)
    ...
    for word in words:
        ...
        content_imgs.append(content_img.to(device))
    ...
    plt.tight_layout()
    vis_path = f"fm_vis_text/epoch_{epoch}_fewshot.png"

...
from networks.encoder import StyleEncoder, ContentEncoder, CrossAttnFuser, ConcatFuser
...
def train_flow_matching_model(args: ScriptArguments):
    ...
    for epoch in range(args.starting_epoch, args.n_epochs):
        ...
        for batch in pbar:
            ...
            # === Fuse ===
            cond_feat = fuser(style_emb, content_feat)                   # B x F x h x w
        ...
        val_mse, val_ctc = validate(
            unet, style_encoder, content_encoder, fuser,
...
def validate(unet, style_encoder, content_encoder, fuser, path_sampler, recognizer, val_loader, device, args):
    unet.eval()
    style_encoder.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader):
            ...
            style_input = x_1
            style_emb = style_encoder(style_input)
            content_feat = content_encoder(content_img)
            cond_feat = fuser(style_emb, content_feat)
...
def plot_few_shot_style_transfer(args, epoch, num_samples=5, k=2):
    ...
    # Load models
    unet = args.unet.eval()
    style_encoder = args.style_encoder.eval()
    content_encoder = args.content_encoder.eval()
    fuser = args.fuser.eval()
    device = torch.device("cuda" if torch.cuda.is_available() else "mps")
    ...
    for row, wid in enumerate(selected_wids):
        ...
        for j in range(num_samples):
            cond_feat = fuser(style,contnet_vecs[j]).to(device)
